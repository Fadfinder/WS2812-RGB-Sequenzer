...
String liveGroups = "";
uint8_t liveStepBrightness = 100;
...
uint32_t blendColor(uint32_t from, uint32_t to, float amount) {
  ...
  uint8_t r = ((from >> 16) & 0xFF) + (((to >> 16) & 0xFF) - ((from >> 16) & 0xFF)) * amount;
  uint8_t g = ((from >> 8) & 0xFF) + (((to >> 8) & 0xFF) - ((from >> 8) & 0xFF)) * amount;
  uint8_t b = (from & 0xFF) + ((to & 0xFF) - (from & 0xFF)) * amount;
  return strip.Color(r, g, b);
}

uint32_t timedNoise(uint16_t led, unsigned long phase, uint16_t salt) {
  uint32_t value = phase ^ ((uint32_t)led * 2654435761UL) ^ ((uint32_t)salt * 2246822519UL);
  value ^= value >> 16;
  value *= 2246822519UL;
  value ^= value >> 13;
  return value;
...
uint32_t effectColorBase(const String& type, uint16_t led, float speed, float speedSetting, uint32_t chosenColor, uint32_t baseColor) {
  unsigned long t = (unsigned long)(millis() * speed);
  unsigned long noisePhase = t / 90UL;
  uint32_t noise = timedNoise(led, noisePhase, type.length() * 31 + type[0]);
  if (type == "fire") {
    return strip.Color(120 + noise % 136, 18 + (noise >> 8) % 94, (noise >> 16) % 9);
  }
  if (type == "storm") {
    if (noise % 100 > 92) return ((noise >> 8) % 100) > 35 ? strip.Color(255, 255, 255) : strip.Color(159, 216, 255);
    uint8_t pulse = 18 + (noise >> 16) % 43;
  ...
  if (type == "welder") {
    bool burst = ((t / 70) % 9) < 4 || noise % 100 > 78;
    if (!burst) return strip.Color(2, 6, 17);
    return ((noise >> 8) % 100) > 28 ? strip.Color(120 + noise % 81, 180 + (noise >> 8) % 76, 180 + (noise >> 16) % 76) : strip.Color(255, 255, 255);
  }
  if (type == "camera") {
  ...
  if (type == "sparkle") {
    return noise % 100 > 82 ? chosenColor : strip.Color(6, 6, 6);
  }
  if (type == "comet") {
  ...
  if (type == "randomOnOff") {
    return noise % 100 > 58 ? chosenColor : 0;
  }
  if (type == "randomColor") {
    return strip.gamma32(strip.ColorHSV(noise & 0xFFFF, 230, 255));
  }
  if (type == "randomOnOffColor") {
    return noise % 100 > 58 ? strip.gamma32(strip.ColorHSV((noise >> 8) & 0xFFFF, 230, 255)) : 0;
  }
  if (type == "meteor") {
  ...
  if (type == "twinkle") {
    uint8_t flash = noise % 100 > 96 ? 255 : (uint8_t)((sin(t / 210.0 + led * 2.17) + 1.0) * 35);
    return scaleColor(chosenColor, flash / 255.0);
  }
  if (type == "candle") {
    uint8_t v = 140 + noise % 116;
    return strip.Color(v, (105 + (noise >> 8) % 56) * v / 255, 18 * v / 255);
  }
  if (type == "sunrise") {
  ...
  if (type == "confetti") {
    return noise % 100 > 72 ? strip.gamma32(strip.ColorHSV((noise >> 8) & 0xFFFF, 230, 255)) : 0;
  }
  if (type == "aurora") {
  ...
  if (type == "toxic") {
    uint8_t v = noise % 100 > 90 ? 255 : 115 + (uint8_t)((sin(t / 260.0 + led) + 1.0) * 55);
  ...
  if (type == "transition") {
    float progress = (millis() - effectStepStartedAt) / (float)max(1UL, effectStepDurationMs);
    return blendColor(baseColor, chosenColor, constrain(progress, 0.0, 1.0));
  }
  if (type == "blink") {
    return ((t / 450UL) % 2) == 0 ? chosenColor : 0;
  }
  if (type == "running") {
    int head = (t / 100UL) % max((uint16_t)1, ledCount);
    return ((led - 1) == head) ? chosenColor : 0;
  }
  return 0;
}

uint32_t effectColor(const String& type, uint16_t led, float speed, float speedSetting, uint32_t chosenColor, uint32_t baseColor, float brightness) {
  return scaleColor(effectColorBase(type, led, speed, speedSetting, chosenColor, baseColor), constrain(brightness, 0.0, 1.0));
}
...
void applyEffectSelection(const String& selection, const String& type, float speed, float speedSetting, uint32_t chosenColor, float brightness) {
  ...
  while (cursor < text.length()) {
    ...
    for (int led = first; led <= last; led++) {
      if (led >= 1 && led <= ledCount) {
        uint32_t baseColor = strip.getPixelColor(led - 1);
        strip.setPixelColor(led - 1, effectColor(type, led, speed, speedSetting, chosenColor, baseColor, brightness));
      }
    }
...
void applyStepGroups(const String& groups, float stepBrightness = 1.0) {
  strip.clear();
  ...
  while (cursor < groups.length()) {
    ...
    if (group.startsWith("FX:")) {
      ...
      if (sep > 3) {
        ...
        applyEffectSelection(leds, type, effectSpeedFactor(speedSetting), speedSetting, color, brightness);
      }
    } else {
      int sep = group.indexOf(':');
      if (sep > 0) {
      String colorPart = group.substring(0, sep);
      float brightness = 1.0;
      int brightnessSep = colorPart.indexOf('@');
      if (brightnessSep > 0) {
        brightness = constrain(colorPart.substring(brightnessSep + 1).toFloat(), 0.0, 1.0);
        colorPart = colorPart.substring(0, brightnessSep);
      }
      uint32_t color = parseColor(colorPart);
      String leds = group.substring(sep + 1);
      applyLedSelection(leds, color, brightness);
      }
    }
  ...
  stepBrightness = constrain(stepBrightness, 0.0, 1.0);
  if (stepBrightness < 0.999) {
    for (uint16_t i = 0; i < ledCount; i++) strip.setPixelColor(i, scaleColor(strip.getPixelColor(i), stepBrightness));
  }
  showStrip();
}
...
void addStep(String line) {
  ...
  uint8_t stepBrightness = 100;
  String option = sep2 > sep1 ? line.substring(sep2 + 1) : "";
  option.trim();
  bool hasOptions = option.startsWith("RANDOM:") || option.startsWith("STEP:") || option.startsWith("BRIGHTNESS:");
  ...
  while (hasOptions && optionStart < option.length()) {
    ...
    if (item.startsWith("RANDOM:")) {
      ...
      item.replace("BRIGHTNESS:", "");
      stepBrightness = constrain(item.toInt(), 0, 100);
    }
  ...
  steps[totalStepCount++] = {duration, randomMaxMs, stepBrightness, groups};
...
void startStep(Step& step) {
  ...
  applyStepGroups(step.groups, step.brightness / 100.0);
}
...
String demoSequenceBlock(bool enabled = true) {
  ...
  block += String(
    ...
    "2000||STEP:Pause\n"
    "7000|FX:fadeIn:50:FFFFFF:1|STEP:Fade-In\n"
    "2000||STEP:Pause\n"
    "7000|FX:fadeOut:50:FFFFFF:1|STEP:Fade-Out\n"
    "2000||STEP:Pause\n"
    "7000|FX:transition:50:FFFFFF:1|STEP:Farbuebergang\n"
    "2000||STEP:Pause\n"
    "7000|FX:blink:50:FFFFFF:1|STEP:Blinklicht\n"
    "2000||STEP:Pause\n"
    "7000|FX:running:50:FFFFFF:1|STEP:Lauflicht\n"
  );
  return block;
}
...
function parseStored(text) {
  ...
  for (const raw of text.split('\n')) {
    ...
    if (parts.length >= 2 && current) {
      ...
      const brightnessOption = options.find(part => part.startsWith('BRIGHTNESS:')) || '';
      current.steps.push({
        ...
        groups: parts[1].split(';').filter(Boolean).filter(group => !group.startsWith('FX:')).map(group => {
          ...
          return { color: `#${colorParts[0]}`, brightness: colorParts.length > 1 ? clamp(Number(colorParts[1]), 0, 1) : 1, leds: group.slice(sep + 1) };
        }),
        effects: parts[1].split(';').filter(group => group.startsWith('FX:')).map(group => {
          const parts = group.split(':');
          if (parts.length >= 6) return { type: parts[1] || 'fire', speed: clamp(readNumber(parts[2], 50), 0, 100), color: `#${parts[3] || 'ffffff'}`, brightness: clamp(readNumber(parts[4], 1), 0, 1), leds: parts.slice(5).join(':') || '' };
          if (parts.length >= 5) return { type: parts[1] || 'fire', speed: clamp(readNumber(parts[2], 50), 0, 100), color: `#${parts[3] || 'ffffff'}`, brightness: 1, leds: parts.slice(4).join(':') || '' };
          if (parts.length >= 4) return { type: parts[1] || 'fire', speed: clamp(readNumber(parts[2], 50), 0, 100), color: '#ffffff', brightness: 1, leds: parts.slice(3).join(':') || '' };
...
function serialize() {
  ...
  const body = playlist.map(seq => {
    ...
    const lines = seq.steps.map(step => {
      ...
      return `${Math.max(20, Math.round(step.durationMs))}|${groups}${randomPart}${namePart}${brightnessPart}`;
    });
...
function updateEffectSpeed(index, value) {
  ...
  currentEffects[index].speed = clamp(readNumber(value, 0), 0, 100);
  renderStrip();
  liveCurrentStep();
}
...
function updateEffectBrightness(index, value) {
  if (!currentEffects[index]) return;
  currentEffects[index].brightness = clamp(readNumber(value, 0) / 100, 0, 1);
  renderStrip();
  liveCurrentStep();
}
...
function liveCurrentStep() {
  ...
  liveTimer = setTimeout(() => {
    const groups = groupsFromMap(paintedColors).map(group => `${group.color.replace('#', '').toUpperCase()}@${clamp(Number(group.brightness ?? 1), 0, 1)}:${group.leds || ''}`).join(';');
    ...
    const params = new URLSearchParams({ groups: allGroups, durationMs: String(secondsToMs(stepSeconds.value, 1)), brightness: String(Math.round(clamp(readNumber(stepBrightness.value, 100), 0, 100))) });
...
void handleLive() {
  ...
  liveGroups = groups;
  liveStepBrightness = constrain(server.arg("brightness").toInt(), 0, 100);
  if (!server.hasArg("brightness")) liveStepBrightness = 100;
  liveUntil = playbackRunning ? millis() + 3000 : 0;
  effectStepDurationMs = constrain(server.arg("durationMs").toInt(), 20, 60000);
  effectStepStartedAt = millis();
  applyStepGroups(groups, liveStepBrightness / 100.0);
  lastEffectAt = effectStepStartedAt;
  addDefaultHeaders();
...
void handleStop() {
  ...
  if (liveGroups.length() > 0) {
    applyStepGroups(liveGroups, liveStepBrightness / 100.0);
  }
  addDefaultHeaders();
